...
/*   Updated: 2024/02/10 20:42:05 by mevangel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	ft_init_sdata(int argc, char **argv, t_data *data)
{
	...
	if (pthread_mutex_init(&data->lock_time, NULL) != 0)
		return (pthread_mutex_destroy(&data->lock_dead),
			pthread_mutex_destroy(&data->lock_done), pthread_mutex_destroy(&data->lock_print),
			printf(ERROR "pthread_mutex_init for data failed.\n"), false);
	return (true);
}
...
static bool	ft_init_sphilo(t_data *data)
{
	...
	if (data->philo == NULL)
		return (ft_exit("malloc failed", data, 0, false), false);
	while (++id <= data->num_philos)
	{
		data->philo[id - 1].id = id;
		data->philo[id - 1].thread = 0;
		data->philo[id - 1].is_full = false;
...
static bool	ft_threads(t_data *data)
{
	...
	while (++i < data->num_philos)
		if (pthread_create(&(data->philo[i].thread), NULL, &philo_routine,
				&data->philo[i]))
			return (ft_exit("pthread_create failed", data, i, true), false);
