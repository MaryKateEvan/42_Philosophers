...
/*   Updated: 2024/02/17 16:28:17 by mevangel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_philo_data(t_philo *philo, int id, t_data *data)
{
	...
	philo->thread = 0;
	philo->times_ate = 0;
	philo->is_done = false;//! i don't need it after all i think
	...
	philo->lock_time = sem_open(philo->time_name, O_CREAT, 0644, 1);
	...
	philo->sem_is_done = sem_open(philo->done_name, O_CREAT, 0644, 0);
...
// 	printf("time name of philo %d is: %s\n", id, philo->time_name);
// 	printf("done name of philo %d is: %s\n", id, philo->done_name);
}
